...
export class MenuService {
  ...
  public getVerticalMenuItems():Array<Menu> {
    let role = this._accountServices.getAccountRole();

    return verticalMenuItems.filter(i => i.role == "*" || i.role.split(";").some(r => this._accountServices.getRoleByCode(r) == role));
  }

  public getHorizontalMenuItems():Array<Menu> {
    let role = this._accountServices.getAccountRole();
    
    return horizontalMenuItems.filter(i => i.role == "*" || i.role.split(";").some(r => this._accountServices.getRoleByCode(r) == role));
  }
