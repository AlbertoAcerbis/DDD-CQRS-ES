...
namespace Muflone.CommonDomain.Core
{
    ...
    public class ConflictDetector : IDetectConflicts
    {
        ...
        {
            if (!this._actions.TryGetValue(typeof(TUncommitted), out var inner))
            {
                    this._actions[typeof(TUncommitted)] = inner = new Dictionary<Type, ConflictPredicate>();
            }
        ...
        {
            if (!this._actions.TryGetValue(uncommitted.GetType(), out var registration))
            {
